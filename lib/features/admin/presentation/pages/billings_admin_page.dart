...
({String label, Color color}) _statusStyle(String status) {
  switch (status) {
    case 'paid':
      return (label: 'Paid', color: const Color(0xFF05CD99));
    case 'overdue':
      return (label: 'Overdue', color: const Color(0xFFFFB547));
    case 'unpaid':
    default:
      return (label: 'Unpaid', color: const Color(0xFFEE5D50));
  }
}
...
class _BillingsAdminPageState extends ConsumerState<BillingsAdminPage> {
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildDetailItem('Period', billing.period?.isNotEmpty == true ? billing.period! : '-'),
      ...
      child: Column(
        ...
        children: [
          ...
          if (statusColor != null)
            ...
            Text(value, style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF2B3674), fontSize: 16)),
        ],
      ),
    );
  }
  ...
  void _showForm({Billing? billing}) {
    ...
    if (residents.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No residents with an assigned house. Assign a house to a resident first.')),
      );
      return;
    }

    final defaultInvoice = 'INV-${DateFormat('yyyyMMdd-HHmmss').format(DateTime.now())}';
    final invoiceController = TextEditingController(text: billing?.invoiceNumber ?? defaultInvoice);
    final titleController = TextEditingController(text: billing?.title ?? 'Monthly Maintenance');
    final amountController = TextEditingController(text: billing != null ? billing.amount.toStringAsFixed(0) : '');
    final periodController = TextEditingController(text: billing?.period ?? '');

    String? selectedResidentId = (billing != null && residents.any((r) => r.id == billing.residentId))
        ? billing.residentId
        : null;
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    _buildTextField(titleController, 'Title (e.g. Monthly Maintenance)', Icons.title),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: residents
                          ...
                          .toList(),
                      onChanged: (val) => setDialogState(() => selectedResidentId = val),
                    ),
                    _buildTextField(amountController, 'Amount (numbers only)', Icons.attach_money, isNumeric: true),
                    _buildTextField(periodController, 'Period (e.g. June 2026) — optional', Icons.event_note),
                    ...
                    InkWell(
                      ...
                      child: InputDecorator(
                        ...
                        child: Text(DateFormat('MMM dd, yyyy').format(dueDate), style: const TextStyle(color: Color(0xFF2B3674))),
                      ),
                    ),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Wrap(
                            spacing: 8,
                            children: [
                              _statusChip('Unpaid', 'unpaid', status, const Color(0xFFEE5D50), (v) => setDialogState(() => status = v)),
                              _statusChip('Paid', 'paid', status, const Color(0xFF05CD99), (v) => setDialogState(() => status = v)),
                              _statusChip('Overdue', 'overdue', status, const Color(0xFFFFB547), (v) => setDialogState(() => status = v)),
                            ],
                          ),
                        ),
                      ],
  ...
  Widget _statusChip(String label, String value, String selected, Color color, ValueChanged<String> onSelect) {
    return ChoiceChip(
      label: Text(label),
  ...
  InputDecoration _inputDecoration(String label, IconData icon) {
    return InputDecoration(
      ...
      border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFFE0E5F2))),
      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFFE0E5F2))),
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFF4318FF))),
    );
  }
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text('Are you sure you want to delete ${billing.invoiceNumber}? This action cannot be undone.'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('Cancel', style: TextStyle(color: Colors.grey)),
            ...
            TextButton(
              onPressed: () async {
                final navigator = Navigator.of(context);
                try {
                  await ref.read(adminBillingsProvider.notifier).deleteBilling(billing.id);
                  navigator.pop();
                } catch (e) {
                  _showError(e);
                }
                                    ...
                                    child: ListView.builder(
                                      ...
                                      itemBuilder: (context, index) {
                                        ...
                                        return Container(
                                          ...
                                          child: Row(
                                            children: [
                                              Expanded(flex: 3, child: Text(b.invoiceNumber, style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF2B3674)))),
                                              Expanded(flex: 3, child: Text(b.resident?.fullName ?? '-', style: const TextStyle(color: Color(0xFF2B3674)))),
                                              Expanded(flex: 2, child: Text(_formatAmount(b.amount), style: const TextStyle(color: Color(0xFF2B3674)))),
                                              Expanded(flex: 2, child: Text(_formatDate(b.dueDate), style: const TextStyle(color: Color(0xFF2B3674)))),
                                              Expanded(
                                                flex: 2,
                                                child: Align(
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: Text(status.label, style: TextStyle(color: status.color, fontWeight: FontWeight.bold, fontSize: 12)),
                                                  ),
                                                ),
                                              ),
                                              Expanded(
